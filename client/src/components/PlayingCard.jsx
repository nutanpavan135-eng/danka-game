export default function PlayingCard({ card, hidden = false, backStatus = "blind" }) {
  if (hidden || !card) {
    return (
      <div className={`card premium-card card-back clean-card-back animated-card back-${backStatus}`} aria-label="Hidden Danka card">
  ...
  return (
    <div className={`card premium-card face-card clean-face-card animated-card ${red ? 'red' : 'black'}`} aria-label={`${label}${suit}`}>
